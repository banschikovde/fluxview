...
| `--git-source-ssh-known-hosts` | build, diff, validate | known_hosts file for SSH host key verification of external git sources; empty = `~/.ssh/known_hosts` plus `/etc/ssh/ssh_known_hosts` (default: `$FLUXVIEW_GIT_SSH_KNOWN_HOSTS`) |
| `--git-source-ssh-accept-new` | build, diff, validate | Accept unknown SSH host keys of external git sources on first use — TOFU, remembered in memory for the run, known_hosts files are never written (default: `false`, env: `FLUXVIEW_GIT_SSH_ACCEPT_NEW`) |
...
- **Scope**: repositories over http(s)/ssh/git/file — public ones fetch anonymously, private ones authenticate from the machine environment (see **Authentication** below). `spec.secretRef`, `spec.proxySecretRef` and `spec.verify` are ignored; `spec.ignore` is not applied (it shapes the source-controller artifact, not the result of building `spec.path` against a full clone). OCIRepository/Bucket sources are not fetched. Path resolution is source-first like Flux: an external GitRepository means `spec.path` always comes from the upstream clone, even when a same-name directory exists locally (a fetch failure then warns and skips in build/diff, and fails validate — the local copy is a different repository's content and is never a fallback).
- **Authentication (private sources)**: a private upstream authenticates like native git on the same machine — from the environment, never from the manifests (`spec.secretRef` is ignored: there is no cluster to read a Secret from; credentials embedded in a URL are ignored too). For `ssh://` and scp-style (`git@host:path`) URLs the SSH key comes, in order, from `FLUXVIEW_GIT_SSH_KEY` (path to a private key file), a live ssh-agent (`SSH_AUTH_SOCK`; an agent holding no keys is skipped), or the first of `~/.ssh/id_ed25519`, `id_ecdsa`, `id_rsa`; an encrypted key opens with `FLUXVIEW_GIT_SSH_PASSPHRASE`, and the SSH user defaults to `git`. Host keys are always verified against known_hosts — the file named by `FLUXVIEW_GIT_SSH_KNOWN_HOSTS`, else `~/.ssh/known_hosts` plus `/etc/ssh/ssh_known_hosts`; an unknown or changed host key is a hard error, and `FLUXVIEW_GIT_SSH_ACCEPT_NEW=1` opts into OpenSSH-style accept-new TOFU (accepted keys are remembered in memory for the run only — known_hosts files are never written, keeping CI deterministic). For `https://` URLs basic auth comes from `FLUXVIEW_GIT_USERNAME`/`FLUXVIEW_GIT_PASSWORD`, or from `FLUXVIEW_GIT_TOKEN` (sugar for forge PATs: `oauth2:<token>`), with `~/.netrc` as the fallback — parsed by fluxview itself, because go-git has no netrc support. Env credentials (and the netrc `default` stanza once the list is set) travel **only to the hosts listed in `FLUXVIEW_GIT_CREDENTIAL_HOSTS`** (CSV, case-insensitive): a `spec.url` comes from the manifest under review — untrusted input, possibly from a fork — so a CI token must never be sent to a host the user did not explicitly name. An empty or unset list applies the credentials nowhere (fail-closed); a netrc `machine` entry keeps working for its own host regardless of the list, and while no list is set the netrc `default` stanza keeps its legacy match-everything behavior. https fetches honor the standard `HTTP(S)_PROXY` environment (go-git built-in); ssh ProxyJump and `GIT_SSH_COMMAND` are not supported. The two policy knobs also exist as flags — `--git-source-ssh-known-hosts` and `--git-source-ssh-accept-new` override their env vars for the run; the credential variables deliberately have no flags, keeping secrets off command lines (and out of `ps` output and CI logs). The same credentials ride every network operation — the ls-remote resolution of floating refs and the clones alike — and authentication failures fail with distinct, named errors (no credentials / rejected credentials / host key) inside the usual URL+ref wrapper. Credentials live in memory only: the cache stays keyed by the normalized URL (which carries no credentials), and error messages never contain passwords, passphrases or key contents.
- **Private upstreams in CI**: hand the job a deploy key and the forge's host key, and fluxview authenticates exactly like on a developer machine:

  ```yaml
  fluxview:validate:
    image: ghcr.io/banschikovde/fluxview:latest
    variables:
      FLUXVIEW_GIT_SSH_KEY: /builds/deploy-key       # masked CI file variable
    script:
      - ssh-keyscan gitlab.corp > ci-known-hosts      # pin the forge's host key — all key types,
                                                      # not `ssh-keyscan -t ed25519`: the KEX may
                                                      # pick another algorithm, and a known_hosts
                                                      # holding only one type then reads the
                                                      # server's other key as a change (MITM error)
      - fluxview validate --path clusters/prod/flux
          --git-source-ssh-known-hosts ci-known-hosts
  ```

  For https upstreams a masked `FLUXVIEW_GIT_TOKEN` (GitLab/GitHub PAT) or `FLUXVIEW_GIT_USERNAME`/`FLUXVIEW_GIT_PASSWORD` covers basic auth — scope it with `FLUXVIEW_GIT_CREDENTIAL_HOSTS: gitlab.corp` so the token cannot travel to a host named by a reviewed manifest. Prefer pinned refs (`ref.tag`/`ref.commit`) in CI: pinned clones are immutable, so a warm mounted cache keeps the run's network footprint to the very first clone.
...
- **Inside the checkout**: the cache may live under the repository working tree (the common CI pattern of `.cache-fluxview/` inside the job checkout). Tree walks never cross a repository boundary — `.git` directories and any nested git repository root (every `data/<…>` clone carries a `.git`) are skipped by resource parsing, kustomize discovery, the remote-ref prefetch scan, loose-file reads and CRD schema conversion (`--schema-dir`) — so cached upstreams stay invisible to builds regardless of where the cache is placed.
