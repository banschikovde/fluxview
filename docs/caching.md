...
- **Private upstreams in CI**: hand the job a deploy key and the forge's host key, and fluxview authenticates exactly like on a developer machine:
  ...
  For https upstreams a masked `FLUXVIEW_GIT_TOKEN` (GitLab/GitHub PAT) or `FLUXVIEW_GIT_USERNAME`/`FLUXVIEW_GIT_PASSWORD` covers basic auth. Prefer pinned refs (`ref.tag`/`ref.commit`) in CI: pinned clones are immutable, so a warm mounted cache keeps the run's network footprint to the very first clone.
