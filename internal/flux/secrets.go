...
// HasSecrets checks if the multi-document YAML contains any Secret resources.
...
// FormatSecretSummary returns a human-readable summary of found secrets.
func FormatSecretSummary(count int) string {
	if count == 0 {
		return ""
	}
	return fmt.Sprintf("⚠ %d secret(s) found and redacted (data replaced with %s)", count, SecretRedactedValue)
}
