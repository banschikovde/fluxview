// Package git provides local git operations for flux-diff using the go-git SDK.
package git
...
func (g *Operations) CloneToDir(_ context.Context, revision string) (string, error) {
	...
	tmpDir, err := os.MkdirTemp("", "flux-diff-revision-*")
	if err != nil {
...
func CloneExternalRepo(ctx context.Context, url, ref string) (string, error) {
	tmpDir, err := os.MkdirTemp("", "flux-diff-external-*")
	if err != nil {
		return "", fmt.Errorf("creating temp dir: %w", err)
	}
