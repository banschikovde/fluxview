...
func TestStripAttrsRecursive(t *testing.T) {
	...
	attrs := map[string]bool{"status": true, "creationTimestamp": true, "helm.sh/chart": true}
	stripAttrsRecursive(m, attrs)

	if _, ok := m["status"]; ok {
		t.Error("expected 'status' to be removed")
	}
	if _, ok := m["spec"]; !ok {
		t.Error("expected 'spec' to be kept")
	}
	...
	if _, ok := ann["helm.sh/chart"]; ok {
		t.Error("expected 'helm.sh/chart' to be removed")
	}
	if _, ok := ann["app.kubernetes.io/name"]; !ok {
		t.Error("expected 'app.kubernetes.io/name' to be kept")
	}
}

func TestStripAttrsRecursive_NestedArray(t *testing.T) {
	m := map[string]interface{}{
	...
	attrs := map[string]bool{"checksum/cm": true}
	stripAttrsRecursive(m, attrs)

	tmpl := m["spec"].(map[string]interface{})["template"].(map[string]interface{})
	meta := tmpl["metadata"].(map[string]interface{})
	ann := meta["annotations"].(map[string]interface{})
	if _, ok := ann["checksum/cm"]; ok {
		t.Error("expected 'checksum/cm' to be removed from nested template annotations")
	}
	if _, ok := ann["kept-key"]; !ok {
		t.Error("expected 'kept-key' to remain")
	}
}
